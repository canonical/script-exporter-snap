...
update source_repo:
  ...
  old_source_tag="$(yq '.parts.script-exporter["source-tag"]' "$snapcraft_file")"
  old_full_version="${old_source_tag#v}"

  full_version="$full_version" yq -i '.version = strenv(full_version)' "$snapcraft_file"
  full_version="$full_version" yq -i '.parts.script-exporter["source-tag"] = "v" + strenv(full_version)' "$snapcraft_file"
